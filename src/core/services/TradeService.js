...
module.exports = class TradeService {
  ...
  async newTrade(req) {
    ...
    const trades = await Promise.all(entries.map(async (entry) => {
      ...
      return this.createTrade(newTradeReq);
    }));
  ...
  async createTrade({
    ...
    const tradeChange = exit.price / entry.price;
  ...
  async score({
    ...
    const tradeDurationDays = (exitTime - entryTime) / (60 * 60 * 24 * 1000);
    const exclusiveChange = tradeChange - marketChange + 1;
    ...
    let outboundChange = exclusiveChange - stdDevPlusMeanChange + 1;
    outboundChange = (outboundChange < 1) ? 1 : outboundChange;
    const inboundChange = exclusiveChange - outboundChange + 1;
    let weightedOutboundChange = Math.log2((outboundChange - 1) * 100) / 100;
    weightedOutboundChange = (weightedOutboundChange > 0) ? weightedOutboundChange : 0;
    const score = inboundChange + weightedOutboundChange;
    const weightedScore = ((score - 1) * weight) + 1;

    return (weightedScore * 100) - 100;
  }
